...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_stok_barang` AS select `t0`.`area_id` AS `area_id`,`t0`.`area_kode` AS `area_kode`,`t0`.`area_nama` AS `area_nama`,`t0`.`brg_id` AS `brg_id`,`t0`.`brg_kode` AS `brg_kode`,`t0`.`brg_nama` AS `brg_nama`,`t0`.`sb_nama` AS `sb_nama`,`db_jsdc`.`stok_barang`.`stok_qty` AS `stok_qty`,`db_jsdc`.`stok_barang`.`stok_limit` AS `stok_limit`,`db_jsdc`.`stok_barang`.`stok_id` AS `stok_id`,`db_jsdc`.`stok_barang`.`stok_time_insert` AS `stok_time_insert`,`db_jsdc`.`stok_barang`.`stok_lasttime_update` AS `stok_lasttime_update`,`db_jsdc`.`stok_barang`.`stok_user_insert` AS `stok_user_insert`,`db_jsdc`.`stok_barang`.`stok_lastuser_update` AS `stok_lastuser_update` from (((select `db_jsdc`.`mt_area`.`area_id` AS `area_id`,`db_jsdc`.`mt_area`.`area_kode` AS `area_kode`,`db_jsdc`.`mt_area`.`area_nama` AS `area_nama`,`db_jsdc`.`mt_barang`.`brg_id` AS `brg_id`,`db_jsdc`.`mt_barang`.`brg_kode` AS `brg_kode`,`db_jsdc`.`mt_barang`.`brg_nama` AS `brg_nama`,`db_jsdc`.`mt_satuan_barang`.`sb_nama` AS `sb_nama` from (`db_jsdc`.`mt_area` join (`db_jsdc`.`mt_barang` left join `db_jsdc`.`mt_satuan_barang` on((`db_jsdc`.`mt_satuan_barang`.`sb_id` = `db_jsdc`.`mt_barang`.`brg_satuan_kecil`)))) where (`db_jsdc`.`mt_barang`.`brg_aset` <> 1))) `t0` left join `db_jsdc`.`stok_barang` on(((`db_jsdc`.`stok_barang`.`stok_area` = `t0`.`area_id`) and (`db_jsdc`.`stok_barang`.`stok_barang` = `t0`.`brg_id`))));
